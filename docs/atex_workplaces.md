...
- **Список и номера (#4612):** в списке только незакрытые задания; выполненные и
  ...
  **№ задания — его место в ПЛАНЕ дня** (по времени планового старта): он проставляется
  на все задания дня разом и не меняется ни от закрытия соседей, ни от скрытия
  выполненных. Сводка «Заданий / Готово / План, шт» тоже считает **весь день**, а не
  показанную часть. Плашка задания — одна строка: слева № и «старт · план · факт»,
  посередине кнопки **✓ Готово / Пропустить**, справа бейдж статуса; у закрытого
  задания кнопок нет (зелёный фон — «Готово», серый — «Пропущена»). Когда закрыты все
  задания дня, на месте списка — приглашение включить показ выполненных.
...
- **Приёмка:** без выбора втулкореза — приглашение выбрать; список = **незакрытые** задания
  втулкореза на выбранную дату по плановому времени, переключатель показывает закрытые
  (они уходят вниз списка, номера при этом у всех прежние); «Готово»/«Пропустить»/
  «Закрыть все» пишут «Закончено»/«Кол-во факт»; в партию записи нет.
