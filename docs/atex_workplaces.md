...
| 12 | Упаковка | Оператор (упаковщик) | Партия ГП, Задание в производство, Событие смены, Упаковочное место | [#4658](https://github.com/ideav/crm/issues/4658) |
...
- **Действия:** по назначенной резке — менять статус (Ожидает → Наладка → В
  ...
  расход партии за новые проходы). Введён весь план — задание завершается, как по
  «✓✓ Готовы все»; введено меньше — остаётся в работе с остатком проходов.
...
- **Приёмка:** оператор выбирает слиттер и дату, открывает смену, затем ведёт
...
- **«События смены» не привязаны к выбору задания (#4610):** секция журнала относится к
  станку и дате, а не к резке, поэтому показывается и до выбора задания — рядом с
  подсказкой «Выберите производственную резку слева». Раньше главная область без
  выбранной резки схлопывалась в одну строку, и журнал пропадал вместе с ней, хотя
  открытие смены, перерывы и отметки прошлых заданий оператору нужно видеть всегда.
  Секции самой резки (раскладка ножей, партии сырья, показания) остаются привязанными к
  выбранному заданию — без него им нечего показывать.
  ![Журнал смены до выбора задания](screenshots/issue-4610-slitter-shift-events.png)
...
- **Список и номера (#4612):** в списке только незакрытые задания; выполненные и
  ...
  **Порядок списка — ПЛАНОВЫЙ** (по времени планового старта), и он же даёт **№ задания**:
  ...
  задания — одна строка: слева № и «старт · план · факт», посередине кнопки
  **✓ Готово / Пропустить**, справа бейдж статуса; у закрытого задания кнопок нет (зелёный
  фон — «Готово», серый — «Пропущена»). Когда закрыты все задания дня, на месте списка —
...
- **Отчёт `sleeve_tasks`:** колонки `task_id`(abn_ID), `task_date`(главное значение, Unix),
  `cutter`/`cutter_id`, `qty`, `fact`, `started`, `finished`. Роль Оператор уже имеет WRITE
...
- **Действия:** калькулятор-визуализатор. По выбранному Виду сырья (ширина
  ...
  каждую карту раскроя, таблицей по ширинам и сводкой. Кнопкой **«В заказ»** план
  можно перенести в Заказ (единственная запись данных).
- **Таблицы (чтение):** `Вид сырья` — поиск и автоподстановка ширины («Ширина, мм»)
  ...
  номинала в факт; `Диаметр втулки` и `Клиент` — справочники для формы «В заказ»;
  `Заказ` — список существующих номеров. Поля ширины/длины редактируемые — можно
  считать и без выбора сырья.
- **Таблицы (запись, только «В заказ»):** `Заказ` (создаётся, если номер новый) и
  по одной `Заказанное количество` на каждую ширину (Вид сырья, Ширина в заказе =
  **номинал**, Длина, Кол-во, Диаметр втулки, Тип намотки).
- **API:** `GET /{db}/metadata` (id таблиц/реквизитов резолвятся по именам — код
  ...
  отчёта — максимум числового номера среди заказов + 1); запись — `_m_new/{Заказ}`
  и `_m_new/{Заказанное количество}?up={orderId}` с токеном XSRF.
...
- **Приёмка:** выбор сырья подставляет ширину/длину; «Рассчитать» показывает до
  трёх карт раскроя (цветная полоса Заказ/Отход), таблицу «ширина (факт) → в
  ...
  общий отход мм). «В заказ» открывает модалку: номер заказа (из списка или новый,
  с подсказкой свободного), для нового заказа — Клиент/Лидер, для позиций — Диаметр
  втулки/Тип намотки; на «Создать» создаются Заказ (если новый) и позиции.
- **Реализация:** шаблон `templates/atex/cut-optimizer.html` (URL
  ...
  ножи, отход, отклонение) вынесено для модульных тестов. Тесты:
  `experiments/atex-cut-optimizer.test.js`, стенд
...
### 3.13 Упаковка (Оператор, планшет)

Упаковщик работает под ролью **Оператор**: у неё уже есть WRITE на `Партию ГП` (1081),
`Событие смены` (1082) и `Задание в производство` (1078) и READ на `Упаковочное место`
(669269) — отдельная роль и новые гранты не нужны, только пункт меню `packer`.
...
- **Действия (#4658):** в правом верхнем углу — **упаковочное место**; оно выбирается один
  ...
  клику на само место (сменить). Ниже — список позиций к упаковке **в порядке отчёта**; по
  позиции жмут **«Упаковано»**: количество подставляется само (`qty_fact`, а если его нет —
  `qty`) и его можно поправить, но тогда **обязательно примечание**. Упакованные скрыты, пока
  не включён переключатель **«Показать упакованные (N)»** (localStorage `atex-pk-show-packed`).
- **Что видит упаковщик:** задание попадает в список, **как только по нему сделана первая
  ...
  объём задания** — сколько проходов отмечено, роли не важно. Карточка: слева **крупный номер
  заказа**, посередине привычная подпись ролика
  ...
  ![Упаковка: список позиций, упакованные показаны зелёным](screenshots/issue-4658-packer-list.png)
...
- **API:** список — `report/packer?JSON_KV&LIMIT=0,5000`; справочник мест —
...
- **Отчёт `packer`:** `task` (главное значение задания, Unix) и `task_id` (abn_ID),
  ...
  `notes` («Примечание»), `events` (COUNT событий смены). Скрытая колонка «Тип события» с
...
- **Приёмка:** первый вход спрашивает упаковочное место (после выбора запрос к таблице
  больше не идёт); список — в порядке отчёта; «Упаковано» с подставленным количеством пишет
  «Упаковано шт» и событие «Упаковка» без станка; правка количества без примечания не
  сохраняется; после перезагрузки упакованные позиции видны упакованными.
- **Реализация:** шаблон `templates/atex/packer.html` (URL `/atex/packer`), логика
  ...
  (разбор строк отчёта, подпись позиции, количество `qty_fact||qty`, требование примечания
  при правке, группировка по заданию, поля записи) покрыто тестами
  `experiments/atex-packer.test.js`. Пункт меню `packer` — у ролей Оператор и Администратор
  (`docs/atex_menu.json`); в боевой базе он заводится записью в таблице Меню (151) под ролью.
