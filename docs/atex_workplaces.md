...
- **Что видит упаковщик:** задание попадает в список, **как только по нему сделана первая
  ...
  места не увидит ни один упаковщик. Карточка: слева **крупный номер
  ...
  ![Упаковка: позиции своего упаковочного места, поправленное количество обведено](screenshots/issue-4680-packer-list.png)
...
- **API:** список — `report/packer?JSON_KV&LIMIT=0,5000&FR_packer_no={номер места}`;
  ...
  углу), а не её id; справочник мест —
...
- **Как устроено.** Токен устройства лежит в `localStorage` под ключом `atehPad`.
  ...
  Запись нашлась — имя планшета уходит в `.navbar-workspace` верхнего меню, и
  только после этого грузится скрипт пульта. Записи нет — на экране «Устройство не
  зарегистрировано».
