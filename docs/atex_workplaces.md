...
- **Действия (#3869, перестроено по образцу Пульта слиттера; затем выравнено под боевую
  ...
  **✓ Готово** («Закончено» = сейчас + «Кол-во факт» = «Кол-во» план) или **Пропустить**
  ...
  незавершённые задания «Готово» (с подтверждением). Завершённые/пропущенные задания
  остаются в списке (бейдж статуса, без кнопок). В партию ничего не пишем.
...
- **Приёмка:** без выбора втулкореза — приглашение выбрать; список = задания втулкореза на
  выбранную дату, активные (по плановому времени) выше завершённых; «Готово»/«Пропустить»/
...
- **Реализация:** шаблон `templates/atex/sleeve-cutter.html`, логика
  ...
  отчёта, поиск таблицы по alias) покрыто тестами `experiments/atex-sleeve-cutter.test.js`.
