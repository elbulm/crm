...
- **Действия:** по назначенной резке — менять статус (Ожидает → Наладка → В
  ...
  #4860: в показаниях счётчика — расход **джамбо**. Джамбо хранится одной партией
  (16–20 км), и помимо нарезанного метража из него тратится протяжка перед резкой
  (**Рабочий расход, м**) и остаток под утиль (**К списанию, м**). Обе величины оператор
  вводит в показаниях вместе с **Номером джамбо** — они прибавляются к разнице
  значений счётчика («Погонаж факт» и расход сырья = резка + рабочий расход +
  к списанию), конечная длина джамбо получается за их вычетом, и партия списывается
  на всё потребление. Запись живёт в таблице «Номер джамбо» (82374, up = задание,
  схема — atex_data_schema.md §6.7): пишется автосохранением при выходе из ячеек и
  финально при завершении резки (длины и число резок — уже фактические); её видит
  упаковщик, расход и списание без номера не сохраняются. Почему «Погонаж
  факт» не кратен метражу×резкам: это разница счётчиков — фактическое потребление
  джамбо, а не плановый метраж.
...
- **Компактный экран пульта (#4783):** пульт занимает рабочую область целиком, и
  ...
  - **дата и станок — в шапке страницы** (`.navbar-workspace`): «{имя планшета} · {дата} ·
    ...
    пульта; временный тестовый вход — ниже, #4808), станок — кнопка, открывающая список станков. Выбор станка помнится в
...
- **День пульта из адреса (#4808, ВРЕМЕННО, для тестов).** Выбора дня в пульте нет
  (#4783 п.4), а на стенде нужно открыть тот день, на который разложены тестовые
  задания. Поэтому день можно задать строкой запроса:
  `/{db}/slitter?date=20260823` (принимается и ISO — `?date=2026-08-23`).
  Мусор и несуществующая дата (`20260231`) подменой не считаются — пульт молча
  остаётся на сегодня. Маршрутизацию параметр не задевает: `action` сервер берёт из
  пути ДО `?` (`index.php:185`), так что рабочее место открывается как обычно.
  Подменённый день **помечен в шапке** — «23.08.2026 (день из адреса)»: пульт с чужим
  днём выглядит как обычный, и спутать тестовый день с настоящим значит вести смену
  не по тем заданиям.
  **Подменяется только показываемый день.** Отметки времени, уходящие в базу
  (`eventDateTime`), по-прежнему берутся из реального «сейчас» (#4348) — иначе
  параметр в адресе испортил бы «Начато»/«Закончено» и хронологию событий смены.
  Разбор — чистая `core.dateFromQuery` (тест
  `experiments/atex-slitter-4808-date-query.test.js`).
...
- **Действия (#3869, перестроено по образцу Пульта слиттера; затем выравнено под боевую
  схему ateh):** смену не открываем. На экране — список заданий на втулки выбранного
  втулкореза, запланированных **на текущий день**. По заданию: **✓ Готово** («Закончено» =
  сейчас + «Кол-во факт» = «Кол-во» план) или **Пропустить** («Закончено» = сейчас, факт не
  трогаем); в сводке дня — **✓✓ Закрыть все**: пометить все незавершённые задания «Готово»
  (с подтверждением). В партию ничего не пишем.
- **Полноэкранный пульт (#4798):** пульт занимает рабочую область целиком — левого меню на
  странице нет (`sleeve-cutter` в списке `FULLSCREEN_ACTIONS`, head `templates/atex/main.html`;
  решение принимается до отрисовки body, иначе меню мелькает). Прокручивается ТОЛЬКО список
  заданий (`.atex-sc-main`). **Даты и выбора втулкореза в форме нет:** они стоя́т в шапке
  страницы (`.navbar-workspace`) подписью «{имя планшета} · {окно плана} · {втулкорез}»
  (`core.workspaceTitleParts`). Втулкорез — кнопка в шапке: открывает список справочника
  модалкой. Втулкорез не выбран — подпись говорит «Втулкорез не выбран», иначе непонятно,
  почему список пуст. Тот же приём, что у пульта слиттера (§3.5, #4783).
- **Окно плана (#4861):** пульт показывает **вчера · сегодня · ещё 2 заполненных дня** —
  вчерашние незакрытые задания больше не исчезают молча (боевое: заказ 5098, задача стоит
  на дне резки 02.09, а пульт 03.09 её уже не показывал). Вчера и сегодня входят в окно
  **всегда, даже пустые** («заданий нет» — тоже информация); впереди берутся **только дни
  с заданиями этого втулкореза** — выходной или день без заданий пропускается вовсе, без
  дырок в списке. Горизонт поиска заполненных дней — 14 суток вперёд
  (`core.planDays`, чтение отчёта одним запросом за окно — `core.windowBoundsUnix`).
  Список сгруппирован по дням: у дня заголовок («Вчера, 02.09», «Сегодня, 03.09»,
  «сб, 05.09»), сводка дня и **своё «✓✓ Закрыть все»** — день закрывают, когда он
  кончился, а не завтрашние задания. Нумерация заданий плановая **внутри дня** (№1 —
  самый ранний старт дня). Подпись в шапке — период окна целиком («02.09 – 08.09.2026»).
  Верхняя сводка (заданий / готово / план) и переключатель «Показать выполненные» —
  по всему окну.
- **Время старта на карточке (#4861):** «старт 08:00» — это **плановое время старта
  резки**, из которой создано задание на втулки (втулки нужны к началу резки). Планирование
  строит день от начала смены (обычно 08:00), поэтому у большинства задач дня время
  одинаковое — это не персональное расписание втулкореза; у задач, чья резка стоит позже
  в очереди, время своё (09:16, 12:08, …). После переносов резки время задачи не
  переписывается — его приводит в порядок генерация/сверка задач (#4631).
- **Втулкорез из планшета (#4789):** планшет настроен на втулкорез (таблица «Планшет»,
  §3.14) — пульт открывается сразу на нём, а выбор втулкореза в шапке настраивает сам
...
- **Список и номера (#4612):** в списке только незакрытые задания; выполненные и
  ...
  **Порядок списка — ПЛАНОВЫЙ** (по времени планового старта), и он же даёт **№ задания**:
  ...
  «Заданий / Готово / План, шт» считает **весь день** (а с #4861 — свой показатель у
  каждого дня и итог по окну), а не показанную часть. Плашка
  ...
  **«45 шт»** — запланированное количество втулок. У закрытого задания кнопок нет (зелёный
  ...
  непонятно, чем оно кончилось. Когда закрыты все задания дня, на месте его списка —
  приглашение включить показ выполненных.
- **Что режем и сколько заготовок (#4786).** Заказ, название втулки и число метровых
  ...
  **N = ⌈план × ширина рулона × 1.1 / 1000⌉** (решение заказчика 18.08.2026: палка длиной
  ...
  «заготовок» в строке нет, план остаётся.
- **Доп. втулка удваивает счёт (#4786).** Позиция с заполненной **«Доп. втулкой»**
  («Приклеить», «Вложить», …) берёт ВТОРУЮ втулку на каждый рулон, а план задачи этого не
  учитывает (боевое ateh1 18.08.2026: позиция 626963 — 56 шт, задача на втулки — те же 56).
  Удвоение считает пульт: количество показано **слагаемыми** — «45+45 шт» (решение заказчика
  18.08.2026: видно, откуда взялось число), заготовки считаются под ОБЕ втулки. Сама доп.
  втулка названа в главной строке — **«доп. втулка: Приклеить»**, и **только когда она
  задана**: заполнена она у меньшинства заданий (на ateh1 — 97 из 476), и строка «нет» на
  остальных карточках была бы шумом. Источник — колонка отчёта `add_sleeve`. Источник — колонки отчёта `order_no`, `sleeve`, `position_width`,
...
- **Отчёт `sleeve_tasks`:** колонки `task_id`(abn_ID), `task_date`(главное значение, Unix),
  ...
  позиции), а также `order_no`, `sleeve`, `position_width`, `sleeve_ready`, `add_sleeve` (#4786 — «что
  режем», признак готовой втулки и доп. втулка; `sleeve_ready` приходит буквой `X`/пусто).
...
- **Действия:** калькулятор-визуализатор. По выбранному Виду сырья (ширина
  ...
  подбирает **ОДНУ карту раскроя ножей** (#4804) с минимальным отходом и показывает,
  сколько рулонов получится. Считает по **фактической** ширине полосы (см. ниже).
  Результат визуализируется полосой-картой, таблицей по ширинам и сводкой. Кнопка
  **«В заказ»** деактивирована (#4690): рабочее место только считает и ничего не записывает.
- **Таблицы (чтение):** `Вид сырья` — поиск и автоподстановка ширины («Ширина, мм»)
  ...
  `Максимальный запас` (#4779) — точки запаса; `Материал втулки` (740264, #4804) —
  материал втулки; `Лидер` (1132) — сведение четырёх лидеров формы с записями по подписи
  (#4811); `Заказ` — список существующих номеров.
...
- **Несколько ширин (#4779):** желаемых рулонов может быть сколько угодно —
  ...
  крестиком; последняя строка остаётся пустой. Раскрой считает набор целиком и кладёт
  **все ширины на одну карту** (#4804), поэтому широкий джамбо закрывается сразу.
- **Втулка: диаметр и материал (#4804).** Втулка задаётся двумя понятными полями —
  **Диаметр втулки** (1″ или 0,5″) и **Материал втулки** (справочник `Материал втулки`,
  740264), — а не выбором одной из длинных записей справочника `Диаметр втулки` (8188).
  У **0,5″ материал не спрашивается**: втулки этого диаметра бывают только картонными,
  и поле заменяется подписью «Картон». Конкретная запись 8188 подбирается **под ширину
  полосы**: сначала ГОТОВАЯ втулка ровно на эту ширину («Ширина втулки, мм» = ширине
  полосы), иначе МЕТРОВАЯ (ширина не задана — режется под размер); нет ни той, ни другой
  — втулка не подставляется вовсе, чужую не пишем (`core.resolveSleeve`). Точки запаса
  сверяются по паре «диаметр + материал» той записи, на которую точка ссылается, а не
  по её id (`core.sleeveMatchesChoice`).
- **Четыре лидера (#4811).** В поле «Лидер» калькулятора не весь справочник `Лидер`
  (1132, семь записей), а **четыре** позиции, нужные менеджеру: **MONOCHROME**,
  **MONOCHROME ZNAK**, **Прозрачный** и **Клиентский**. Список захардкожен (решение
  заказчика), а **id записей — нет**: три «своих» лидера сводятся со справочником ПО
  ПОДПИСИ (`core.resolveLeader`), как и все прочие сущности рабочего места. Поэтому
  пересборку базы код переживает, а появись в справочнике запись «Клиентский» — она
  подхватится сама.
  **Клиентский лидер — лидер заказчика:** записи в справочнике у него нет, и **точки
  запаса под него не подбираются вовсе** — нарезать впрок под чужой лидер нечего.
  Панель говорит об этом прямо («точек запаса под него не бывает»), а не показывает
  пустой список: пустота читалась бы как сбой подбора. В «Лидер» заказа и позиции
  (ссылка на 1132) пишется id найденной записи; у «Клиентского» её нет — поле остаётся
  пустым, чужая запись не подставляется.
- **Точки запаса (#4779):** после длины рулона задаются **Лидер** (#4811 — четыре,
  см. выше), втулка (диаметр
  и материал) и **Тип намотки** — на геометрию раскроя они не влияют, а вместе с Видом
...
- **Модель раскроя (#4804):** карта раскроя **одна**, и на ней лежат **все** заданные
  ширины — деления на «Карту 1»/«Карту 2» больше нет. Ножи подбираются так, чтобы
  джамбо было занято максимально плотно:
  1. кладутся **целые пропорциональные наборы** (пропорция желаемых количеств через
     НОД) — приоритет пропорции;
  2. **остаток джамбо добивается** любыми заданными ширинами по минимуму отхода
     (`fillRemainder`, безграничный рюкзак; при равном заполнении побеждает ширина с
     большей долей в пропорции);
  3. если даже ОДИН пропорциональный набор шире джамбо — **пропорция ужимается**, пока
     набор не влезет (`shrinkToFit`), и об этом говорится плашкой над планом. Каждой
     ширине при этом остаётся хотя бы один нож.

  Проходов ровно столько, чтобы выпуск **не был меньше заданного** ни по одной ширине.
  По одному ножу на каждую ширину уже шире джамбо — план не строится и называет причину.
  Показывается **один** вариант — лучший по отходу. **Ничего на склад**: незанятый край
  джамбо это «Отход» (`Отход, мм` = *W* − Σ(ширина × ножей)), а избыток сверх заказа
  показывается как `Δ к желаемому`. Канонический пример: вход 880 → 60×14 + 40×1, отход 0.
  Пример добивки: вход 910, задано 60×5 и 50×10 → 6×60 + 11×50, отход 0.
- **Процент отхода — от эффективной ширины (#4828).** В строке карты «Отход: N мм
  (P%)» база процента — **занятая полосами (эффективная) ширина резки**, а не общая
  рабочая ширина джамбо: менеджеру по нему видно, **на сколько процентов умножить
  цену прайса**, продавая дорезок (вход 891, занято 740, отход 151 → 20,405%, а не
  16,947%). Доля отхода от общей ширины *W* в таком виде важна **планеру** и в
  калькуляторе не отображается (в планировании производства свой расчёт, он не
  менялся). База процента названа во всплывающей подсказке строки.
- **Время на резку — наладка + намотка + лидер (#4832).** Метрика «Время на резку»
  (#3744) считает **наладку 45 мин (единожды) + «Всего резок» × (намотка рулона по
  норме WIND_* + лидер BETWEEN_CUTS, дефолт 2 мин/проход)** — те же нормы из
  «Время операции, мин», что в планировании. Компоненты складываются без
  промежуточных округлений, итог округляется до целых минут один раз
  (round((1,8 намотка + 2 лидер) × 25 проходов + 45) = 140). Формула целиком —
  во всплывающей подсказке метрики.
- **Вид сырья — только с галкой «Для расчета резки» (#4848).** Список «Вида сырья»
  показывает только отмеченные галкой `Для расчета резки` (BOOLEAN, из JSON_OBJ
  приходит `'X'`). Реквизита нет в сборке — список не фильтруется: старая база
  не остаётся с пустым справочником.
- **Приёмка:** выбор сырья подставляет ширину и показывает подходящие точки запаса;
  ...
  показывает ОДНУ карту раскроя (цветная полоса Заказ/Отход), таблицу «ширина (факт) → в
  заказе / желаемое / получится / Δ» и сводку с выделенными цветом главными
  параметрами (Итого рулонов, Общий отход м²) и общими (всего резок, общий отход мм).
  Кнопка «В заказ» видна, приглушена и не нажимается (#4690).
- **Реализация:** шаблон `templates/atex/cut-optimizer.html` (URL
  ...
  фактической ширины, единая карта, пропорция через НОД, добивка остатка, сегменты-
  ножи, отход, отклонение; `core.resolveSleeve` — подбор втулки под ширину полосы;
  `core.matchStockPoints` — отбор точек запаса) вынесено
  для модульных тестов. Тесты:
  `experiments/atex-cut-optimizer.test.js`, `experiments/atex-cut-optimizer-4804.test.js`,
  `experiments/atex-cut-optimizer-4811-leaders.test.js`,
...
- **Скролл списка (#4856):** пульт занимает высоту рабочей области целиком (приём
  пульта слиттера, #4783): шапка с упаковочным местом всегда на виду, прокручивается
  только список позиций (`.atex-pk-main { overflow-y: auto }`).
- **Место из планшета (#4789, #4852):** упаковочное место у упаковщика ОДНО — то, что
  настроено в записи планшета (таблица «Планшет», §3.14): упаковка открывается сразу на
  нём (в «Планшете» место лежит ссылкой или номером, отчёт фильтруется по номеру).
  **Выбора места больше нет (#4852):** в углу — некликабельная плашка места; настроит
  планшет диспетчер в таблице «Планшет». Настройки нет — пульт не откроется вовсе:
  сторож показывает «Планшет не настроен» и не грузит код упаковки (§3.14, #4868).
...
- **Что видит упаковщик:** задание попадает в список, **как только по нему сделана первая
  ...
  номера крупным остаётся внутренний и второй строки нет; с #4799 разрыв между этими двумя
  строками уменьшен — клиентский номер стал мельче, внутренний крупнее, но первый по-прежнему
  больше второго на любой ступени (`is-long`/`is-tiny`). Посередине привычная подпись ролика
  `MWR113L 110 х 600 IN втулка пластик серая Прозрачный + доп. втулка: Приклеить` — **лидер**
  (колонка `leader`, #4799) встаёт следом за втулкой, перед хвостом доп. втулки. Ниже время
  задания и план/факт, а последним в теле карточки — **артикул** (колонка `art`, #4799)
  отдельной плашкой по ширине содержимого (`.atex-pk-art`, не растягивается на всю колонку);
  обе колонки бывают пустыми, тогда карточка просто без них. Справа количество и кнопка. Когда у одного задания несколько позиций, над ними
...
- **Левого меню нет (#4799):** рабочее место занимает экран целиком — тем же способом, что и
  пульт слиттера (§3.5): `packer` добавлен в список `FULLSCREEN_ACTIONS` в `<head>`
  `templates/atex/main.html`, решение принимается до отрисовки body, иначе меню мелькает.
...
**только с зарегистрированного планшета** и только для роли **«Оператор»** (#4868).
Планшеты
...
- **Кому место открыто (#4868).** Сторож проверяет две вещи ещё до загрузки кода
  пульта: роль вошедшего и настройку планшета. Шаблон пульта объявляет их атрибутами
  тега сторожа: `data-pad-roles="1621"` (id роли «Оператор», тот же список, что
  `PAD_ROLE_IDS` в `main.html`) и `data-pad-kind` — какой объект планшета нужен этому
  пульту (`slitter` / `cutter` / `place`, колонки «Слиттер»/«Втулкорез»/«Упаковочное
  место»). Роль не из списка — экран **«Это операторское место»**, планшет есть, а
  колонка объекта пуста — экран **«Планшет не настроен»** с кодом устройства; в обоих
  случаях скрипт пульта не загружается вовсе. Выбор объекта внутри пульта больше не
  заменяет настройку: место планшету задаёт диспетчер в таблице «Планшет».
...
- **Оператор — только в свой пульт (#4852).** Роль планшета (`data-pad-roles`
  в `main.html`) уводится в рабочее место планшета с ЛЮБОЙ страницы базы, не только
  с корня: `pad-home.js` сравнивает открытую страницу с настроенным пультом — чужая
  страница заменяется редиректом, своя (иначе был бы цикл перезагрузок) остаётся,
  настроек нет — экран «Рабочее место не настроено» с кодом устройства. Так оператор,
  вошедший по QR-коду с несконфигурированного телефона, получает эту ошибку на любой
  странице. Упаковочное место в «Упаковке» с тех пор задаётся только планшетом —
  выбор места из пульта убран (#4852).
