...
if(($z === "my") && ((isset($com[2]) ? $com[2] : "") === "register")){ # Register the user
    ...
    elseif(isset($_POST["email"])){
        ...
        if(isset($_COOKIE["yd_param"])){
            ...
            Insert($id, 1, 304, $campId, "Insert the campaign tag");
		}
		else
			$campId = "";
        $db = mail2DB($email, $id);
        # Inform the admin
        mysendmail(ADMINEMAIL, "Registration from $campId ".$_SERVER["SERVER_NAME"].": $email"
...
function Check_Grant($id, $t=0, $grant="WRITE", $fatal=TRUE)	# $fatal stops the script on no access
{
	...
	elseif($t == 0)
		$data_set = Exec_sql("SELECT obj.t, COALESCE(par.t, 1) par_typ, COALESCE(par.id, 1) par_id, COALESCE(arr.id, -1) arr, obj.val ref
								FROM $z obj LEFT JOIN $z par ON obj.up>1 AND par.id=obj.up
									LEFT JOIN $z arr ON arr.up=par.t AND arr.t=obj.t AND arr.t!=arr.up
	...
	elseif($id != 1)
		...
		$data_set = Exec_sql("SELECT $t t, 1 par_typ, 1 par_id, -1 arr, -1 ref", "Get 1st level Object");
#print_r($GLOBALS); die();
...
function Validate_Token(){ # Validates the cookie token and gathers the user permissions
	...
	elseif(isset($dumpAPI))
	{
		...
		foreach(getallheaders() as $key => $value)
    		if(strtolower($key) === "x-authorization" || strtolower($key) === "authorization"){
	...
	{
		$tok = addslashes($_POST["token"]);
		$typ = TOKEN;
	}
	elseif(isset($_COOKIE["idb_$z"]))
	{
		$tok = addslashes($_COOKIE["idb_$z"]);
		$typ = TOKEN;
	}
	elseif(isset($_COOKIE["idb_$z"])){
		$tok = addslashes($_COOKIE["idb_$z"]);
	...
	elseif(isApi()){
		foreach(getallheaders() as $key => $value){
    		...
    		elseif(strtolower($key) === "authorization"){
...
#        print_r($GLOBALS);print_r($GLOBALS["TRACE"]);my_die("ok");
		if($row = mysqli_fetch_array($data_set)){
    		...
    		if(!$row["tok"])
				...
				my_die(t9n("[RU]Неверный токен пользователя GUEST[EN]Invalid token for GUEST")." <a href=\"/$z/login\">login</a>");
    		else
		...
		elseif(isset($_COOKIE["idb_my"])){ // Log in the DB using the cabinet's token
    		$data_set = Exec_sql("SELECT u.val FROM my tok, my u, my db"
        						." WHERE u.t=".USER." AND tok.up=u.id AND tok.val='".addslashes($_COOKIE["idb_my"])."' AND tok.t=".TOKEN
        ...
        if(isset($GLOBALS["REV_BT"][$typ]))
            switch($GLOBALS["REV_BT"][$typ]){
    			...
    			case "NUMBER":
    				...
    				break;
    			case "BOOLEAN":
    			    if(($val === "") || ($val === "0") || (strtolower($val) === "false") || ($val === "-1") || ($val === " "))
    					$val = "";
    				else
    					$val = "1";
    				break;
...
function Format_Val_View($typ, $val, $id=0)
{
	...
	if($val != "NULL" && isset($GLOBALS["REV_BT"][$typ]))
		switch($GLOBALS["REV_BT"][$typ])
		{
			...
			case "GRANT":
				if($val == 0) 
					return (isset($_REQUEST["JSON_OBJ"]) ? "$val:" : "").TYPE_EDITOR;
				if($val == 1)
					return (isset($_REQUEST["JSON_OBJ"]) ? "$val:" : "").ALL_OBJECTS;
				if($val == 10)
					return (isset($_REQUEST["JSON_OBJ"]) ? "$val:" : "").FILES;
			...
			case "REPORT_COLUMN":
				...
				elseif(!isset($GLOBALS["REP_COLS"][$val]))
				{
					$sql = "SELECT a.id, a.val, reqs.id req_id, refs.val req_val, reqs.val attr, ref_vals.val ref_val
							FROM $z a LEFT JOIN ($z reqs CROSS JOIN $z refs) ON refs.id=reqs.t AND reqs.up=a.id AND reqs.t!=reqs.up
				...
				$val = (isset($_REQUEST["JSON_OBJ"]) ? "$val:" : "") . $GLOBALS["REP_COLS"][$val];
				break;
			case "PWD":
...
# Construct the report head
		...
		if($check)
			if(Check_Val_granted(REPORT, $row["val"], $id) === "BARRED")
	    		my_die(t9n("[RU]У вас нет доступа к этому объекту ($val))![EN]You do not have this object granted ($val))")." ($t)");
		...
		if(!isset($GLOBALS["STORED_REPS"][$id]["sql"])){
			$sql = "SELECT distinct CASE WHEN col_def.up=0 THEN col_def.id ELSE col_def.up END typ, reqs.id req, req_refs.t refr, arr_vals.up arr, reqs.val attr
					FROM $z rep LEFT JOIN $z col_def ON col_def.id=rep.val 
						LEFT JOIN $z reqs ON reqs.up=CASE WHEN col_def.up=0 THEN col_def.id ELSE col_def.up END AND reqs.t!=reqs.up
    			...
    			foreach($GLOBALS["STORED_REPS"][$id]["types"] as $key => $typ){ # Col => Type
    			    ...
    			    if(isset($GLOBALS["STORED_REPS"][$id][REP_COL_NAME][$key])){
    			        ...
    			        elseif(isset($_REQUEST["FR_$t"]) && strlen($_REQUEST["FR_$t"]))
                				$GLOBALS["CONDS"][$key]["FR"] = $_REQUEST["FR_$t"];
    			        if(isset($_REQUEST["TO_$str"]) && strlen($_REQUEST["TO_$str"]))
					...
					if(strlen($typ)){ # A real field, not a synthetic (calculatable) one
						...
						if(!isset($master)){  # Master is the Parent of the first column of the report
							...
							if(isset($_REQUEST["i$master"])) # We got an explicit filter for $master table
								...
								$conds[$master] = "$p$master.up!=0 AND $p$master.t!=$p$master.up AND length($p$master.val)!=0 AND $p$master.t=$par";
						}
						unset($repJoin);
...
#			trace("Format $val base:$base - base_str:$base_str id:".$row["i$key"]);
			...
			elseif(isApi())
				$val = strlen($val) ? Format_Val_View($base, $val) : "";
			else
	    ...
	    elseif(isset($_REQUEST["JSON_KV"])){
        	...
        	if(isset($blocks["col_totals"][$id]))
        	    foreach($blocks["col_totals"][$id] as $v){
					if(isset($json["columns"][$i]))
						$json["columns"][$i]["totals"] = $v;
					$i++;
				}
			$json["header"] = $GLOBALS["STORED_REPS"][$id]["header"];
	    }
			...
			switch($GLOBALS["GLOBAL_VARS"]["action"]) # Show the current mode & object in the page header
			{
				...
				case "csv_all":
        			...
        			$sql = "SELECT a.id, a.val, IF(base.t=base.id,0,1) ref, IF(base.t=base.id,defs.val,base.val) req, count(def_reqs.id) req_req, reqs.id req_id, defs.id req_t, defs.t req_base, a.t base
        			            FROM $z a LEFT JOIN $z reqs ON reqs.up=a.id AND reqs.t!=reqs.up LEFT JOIN $z defs ON defs.id=reqs.t LEFT JOIN $z def_reqs ON def_reqs.up=defs.id LEFT JOIN $z base ON base.id=defs.t
				    ...
				    foreach($typ as $i => $v)
				    {
                		...
                		do{
                    		...
                    		if(isset($arr[$i]))
                        		...
                        		$data_set = Exec_sql("SELECT id FROM $z obj WHERE t=$i AND up!=0 AND t!=up AND id>$last ORDER BY id LIMIT $limit", "Get objects for CSV");
                    		if($row = mysqli_fetch_array($data_set)){
        					    $first = $row["id"];
                    	        do{
                    	            $last = $row["id"];
                        		} while($row = mysqli_fetch_array($data_set));
                        		$data_set = Exec_sql("SELECT obj.id, obj.val".$select[$i]." FROM $z obj".$join[$i]." WHERE obj.t=$i AND obj.up!=0 AND obj.t!=obj.up AND obj.id>=$first AND obj.id<=$last"
                        		                    , "Get reqs for CSV");
...
#			Check_Types_Grant(FALSE);
			$data_set = Exec_sql("SELECT typs.id, typs.t, refs.id ref_val, typs.ord uniq
						...
						FROM $z typs LEFT JOIN $z refs ON refs.id=typs.t AND refs.id!=refs.t
							LEFT JOIN $z reqs ON reqs.up=typs.id AND reqs.t!=reqs.up
		...
		case "&object":
			...
			if($row = mysqli_fetch_array($data_set)){
    			...
    			if(isApi()){
    				...
    				$GLOBALS["GLOBAL_VARS"]["api"]["obj"]["val"] = $v;
...
#print_r($GLOBALS);die();
			$data_set = Exec_sql("SELECT gr.id, gr.val, reqs.id req_id, reqs.t req_t, req_typ.val req_val, ref_reqs.val ref_val
									FROM $z gr LEFT JOIN ($z reqs CROSS JOIN $z req_typ) ON gr.id!=1 AND reqs.up=gr.id AND reqs.t!=reqs.up AND req_typ.id=reqs.t
...
#print_r($GLOBALS);die();
				$data_set = Exec_sql("SELECT pars.id par_id, reqs.id req_id, pars.val par_name, pars.t par_base
							...
							FROM $z pars LEFT JOIN $z reqs ON reqs.up=pars.id AND reqs.t!=reqs.up
     			...
     			if(isApi()){ # Prepare the list of dependent terms
        			$data_set = Exec_sql("SELECT a.id, a.val, a.t, reqs.t reqs_t FROM $z a LEFT JOIN $z reqs ON reqs.up=a.id AND reqs.t!=reqs.up
...
#print_r($GLOBALS);print_r($rows);die();
				...
				if(Val_barred_by_mask($key, isset($row["val"]) ? (isset($GLOBALS["REF_typs"][$key]) ? $row["ref_val"] : $v) : NULL))
...
#print_r($GLOBALS);die();
		...
		case "&add_obj_ref_reqs":
			...
			$data_set = Exec_sql("SELECT def_reqs.id, ref_reqs.id ref_req, base.t base, is_ref.val ref_name
									...
									FROM $z r JOIN $z def_reqs ON def_reqs.up=r.t AND def_reqs.t!=def_reqs.up
...
#				trace(" SELECTED? ".$blocks[$blocks[$block]["PARENT"]]["CUR_VARS"]["val"] ."==". $row["id"]);
		...
		case "&uni_obj_list":
		    if(isset($_GET["val"]))
		        ...
		        $cond = "";
			$sql = "SELECT a.id, a.val, a.t, reqs.t reqs_t, reqs.up FROM $z a LEFT JOIN $z reqs ON reqs.up=a.id AND reqs.t!=reqs.up
...
#print_r($GLOBALS);die();
			...
			elseif(isset($_REQUEST["ORDER"])){
				$tmp = (int)$_REQUEST["ORDER"];
				if($tmp < 1){
					$_REQUEST["ORDER"] = abs((int)$_REQUEST["ORDER"]);
					$_REQUEST["desc"] = 1; // Revert the order
				}
				$GLOBALS["ORDER_VAL"] = $_REQUEST["ORDER"]=="$id" ? "val" : (int)$_REQUEST["ORDER"];
			}
			else
				$GLOBALS["ORDER_VAL"] = 0;
			...
			if(isset($_POST["import"])||isset($_GET["import"])){
				...
				if((!isset($GLOBALS["GRANTS"]["EXPORT"][1]) || !isset($GLOBALS["GRANTS"][1]) || $GLOBALS["GRANTS"][1] !== "WRITE")
				    ...
				    if(isset($_POST["F_U"]))
				        ...
				        if($row = mysqli_fetch_array(Exec_sql("SELECT req.id FROM $z par, $z req WHERE par.up=0 AND req.up=par.id AND req.t!=req.up AND req.t=$i AND par.id=".(int)$_POST["autoParent"], "Get req ID")))
...
#				print_r($GLOBALS);die($i);
    ...
    #print_r($GLOBALS);die($i);
    				...
    				if($up != 1)
    				{
    					$data_set = Exec_sql("SELECT reqs.t req, a.t par FROM $z a LEFT JOIN $z reqs ON reqs.up=a.t AND reqs.t!=reqs.up AND reqs.t=$i WHERE a.id=$up"
    										, "Validate Parent ID");
    					if($row = mysqli_fetch_array($data_set))
    					{
    						if($row["req"] != $i)
				...
				if($plain_data){
    				...
    				while(!feof($handle)){
    					...
    					if($getParent){
						    ...
						    if($object[0] === "" && empty($_REQUEST["allowEmptyValue"])){
...
#print($header);#print_r($blocks["typ"]);print_r($blocks["typ"]);print_r($GLOBALS);die();
		...
		case "&filter_req_dns":
			...
			else
			{
				...
				if(isset($_REQUEST["F_$cur_typ"]))
					$blocks[$block]["filter"][] = str_replace("\"", "&#34;", $_REQUEST["F_$cur_typ"]);
				elseif(isset($_REQUEST["FR_$cur_typ"]))
					$blocks[$block]["filter"][] = str_replace("\"", "&#34;", $_REQUEST["FR_$cur_typ"]);
				else
					$blocks[$block]["filter"][] = "";
			}
			break;
		...
		case "&uni_obj_all":
			if(isset($GLOBALS["HAVE_ARR"]))  # Get base types for array Reqs (to simplify the filtering later)
			{
				$data_set = Exec_sql("SELECT arr_reqs.t req_typ, base_typs.t base_typ
										...
										WHERE reqs.up=$id AND reqs.t!=reqs.up AND arr_reqs.up=reqs.t AND base_typs.id=arr_reqs.t"
...
#				$filter_cond = " and vals.id IS NOT NULL"; 
			...
			if(isset($_REQUEST["LIMIT"])){
				$req_limits = explode(",", $_REQUEST["LIMIT"]);
				if(isset($req_limits[1])) # Requested: offset, limit
					$order .= " LIMIT ".(int)$req_limits[0].",".(int)$req_limits[1];
				else
					$order .= " LIMIT $pg ".(int)$req_limits[0];
			}
			...
			if(isset($_GET["_count"])){
				if($row = mysqli_fetch_array(Exec_sql("SELECT COUNT($distinct vals.id) cnt FROM $z vals $filter_tables
                    									WHERE vals.t=$cur_typ AND vals.t!=vals.up $parent_cond $filter_cond", "Count filtered Objs")))
...
#{print($head_str);}#print_r($GLOBALS["data"]);print_r($GLOBALS);die();}
    			...
    			if(isApi()){
					$v = str_replace(["\r\n", "\r", "\n"], '\n', str_replace('"', '\"', str_replace('\\', '\\\\', $row["val"])));
    			    if(isset($_REQUEST["JSON_DATA"]))
        				$GLOBALS["GLOBAL_VARS"]["newapi"][$row["id"]] = "{\"i\":".$row["id"].",\"u\":".$row["up"].",\"o\":".$row["val_ord"].",\"r\":[\"".$v."\"";
    			    elseif(isset($_REQUEST["JSON_OBJ"])){
        				$GLOBALS["GLOBAL_VARS"]["newapi"][] = array("i"=>(int)$row["id"],"u"=>(int)$row["up"],"o"=>(int)$row["val_ord"],"r"=>[Format_Val_View($cur_base_typ, $row["val"], $row["id"])]);
						$GLOBALS["GLOBAL_VARS"]["newapicnt"][$row["id"]] = array_key_last($GLOBALS["GLOBAL_VARS"]["newapi"]);
					}
    			    else{
				...
				&& strlen($filter_cond))
			{
				...
				{
					if($row = mysqli_fetch_array(Exec_sql("SELECT COUNT($distinct vals.id) cnt FROM $z vals $filter_tables
									WHERE vals.t=$cur_typ AND vals.t!=vals.up $parent_cond $filter_cond", "Get number of filtered Objs")))
				...
				else
				{
					...
					$row = mysqli_fetch_array(Exec_sql("SELECT COUNT(1)-(SELECT COUNT(1) cnt FROM $z vals WHERE t=$cur_typ AND up=0) cnt FROM $z vals WHERE t=$cur_typ AND t!=up $filter_cond", "Get number of Objs"));
					#$n = $row["cnt"];
...
#						trace("We got a query for the ".$row["id"]." -> ".$row["ref_id"]." DDL (".$GLOBALS["STORED_REPS"][$ref]["ddl"].") $id\r\n".$GLOBALS["STORED_REPS"][$id]["sql"]);
    					...
    					if($GLOBALS["STORED_REPS"][$id]["sql"]){
    					    ...
    					    if(!isset($GLOBALS["STORED_REPS"][$ref]["result"])){
                    			...
                    			while($refDDL = mysqli_fetch_array($refDDLs, MYSQLI_NUM)){
                    			    $GLOBALS["STORED_REPS"][$ref]["result"][$refDDL[0]] = isset($refDDL[1]) ? $refDDL[1] : (isset($refDDL[2]) ? $refDDL[2] : $refDDL[0]);
    					    ...
    					    if(isset($GLOBALS["STORED_REPS"][$ref]["result"][$row["t"]]))
                    			...
                    			if($refDDL = mysqli_fetch_array(Exec_sql($sql, "Get DDL query"), MYSQLI_NUM)){
                					$row["refr"] = isset($refDDL[1]) ? $refDDL[1] : (isset($refDDL[2]) ? $refDDL[2] : $refDDL[0]);
			...
			foreach($GLOBALS["REQS"] as $key => $value){
				...
				if(isApi()){
    			    ...
    			    if(isset($_REQUEST["JSON_OBJ"])){
    					if(isset($row["ref_id"]))  # Reference
        				    $GLOBALS["GLOBAL_VARS"]["newapi"][$GLOBALS["GLOBAL_VARS"]["newapicnt"][$parent_id]]["r"][] = (is_array($row["ref_id"]) ? implode(",", $row["ref_id"]) : $row["ref_id"]).":$v";
        				else
        				    $GLOBALS["GLOBAL_VARS"]["newapi"][$GLOBALS["GLOBAL_VARS"]["newapicnt"][$parent_id]]["r"][] = $v;
    			    }
				}
				if(isset($_REQUEST["csv"])){
					if(isset($GLOBALS["ARR_typs"][$key])) # Array of linked values
			...
			if(isset($GLOBALS["STORED_REPS"][$id]["head"]))
			{
				foreach($GLOBALS["STORED_REPS"][$id]["head"] as $key => $value)
					...
					{	
						...
						$t = $GLOBALS["STORED_REPS"][$id]["columns"][$key];
						if(isset($_REQUEST["FR_$value"]) && strlen($_REQUEST["FR_$value"])) # First check filter by name
							$blocks[$block]["fr_val"][] = $_REQUEST["FR_$value"];
						elseif(isset($_REQUEST["FR_$t"]) && strlen($_REQUEST["FR_$t"])) # then by its ID
							$blocks[$block]["fr_val"][] = $_REQUEST["FR_$t"];
						elseif(isset($GLOBALS["STORED_REPS"][$id][REP_COL_FROM][$key])) # then by the report settings
                            $blocks[$block]["fr_val"][] = $GLOBALS["STORED_REPS"][$id][REP_COL_FROM][$key]; 
						else
							$blocks[$block]["fr_val"][] = "";
						if(isset($_REQUEST["TO_$value"]) && strlen($_REQUEST["TO_$value"])) # First check filter by name
...
#print_r($GLOBALS); die();
	...
	if(isset($_REQUEST["F_U"]))
			$_REQUEST["F_U"] = BuiltIn($_REQUEST["F_U"]);
	if(isset($_REQUEST["F_I"]))
			$_REQUEST["F_I"] = BuiltIn($_REQUEST["F_I"]);
    $arg = "";
...
#print($GLOBALS["TRACE"]); die($id);
		...
		case "_setorder":
			...
			if($row = mysqli_fetch_array($result)){
			    if("$newOrd" !== $row["ord"]){
    				...
    				$id = $row["up"];
    				$ord = $row["ord"];
    				Exec_sql("UPDATE $z SET ord = CASE 
								WHEN id = $rid THEN LEAST($newOrd, (SELECT * FROM (SELECT MAX(ord) FROM $z WHERE up = $id) AS tmp))
		...
		case "_d_del":
		case "_deleteterm":
			$data_set = Exec_sql("SELECT COUNT(id) FROM $z WHERE t=$id AND t!=up", "Check, if the Typ is being used");
			if($row = mysqli_fetch_array($data_set))
				if($row[0] > 0)
		...
		case "_new_db":
			...
			if($row = mysqli_fetch_array($result))
			    if($row["dbs"] >= 3 && (int)$row["plan"] < 1147)
    			    my_die(t9n("[RU]В бесплатном тарифе доступно не более 3 баз[EN]Maximum 3 DBs available on a free plan"));
            if(!isDbVacant($db))
...
#		        my_die("{\"error\":\"No grants to id $id\"}");
			$data_set = Exec_sql("SELECT dic.t dic, r.val attr, def_reqs.t, req_orig.t base, base.id!=base.t is_ref, req.id req_id
							FROM $z r LEFT JOIN $z dic ON dic.id=r.t
								...
								LEFT JOIN $z def_reqs ON def_reqs.up=r.t AND r.t!=1 AND def_reqs.t!=def_reqs.up
		...
		case "rep_cols":
			...
			$data_set = Exec_sql("SELECT pars.id par_id, reqs.id req_id, pars.val par_name, reqs.val ref_name, NULL cols
							...
							, CASE WHEN req_typs.val='' THEN ref_reqs.t ELSE req_typs.t END base
						FROM $z pars
							LEFT JOIN $z reqs ON reqs.up=pars.id AND reqs.t!=reqs.up
