...
//
// «Заморозка» дня (#4326) обещает: «планирование его не трогает». Вариант A выполнял это ЧАСТИЧНО —
// задания замороженного дня оставались ВО ВХОДЕ планировщика, лишь пришпиленные к своему дню
// (временный `c.fixed` + `dayAnchorByCut`). Упаковщик каждый раз раскладывал день ЗАНОВО встык, и
// «Дата план» заданий менялась (проба: 08:00/09:00/11:40 → 08:00/09:11/13:15) — записи уходили в
// базу. Тем же путём переписывался хранимый тайминг: `applySplitPlan` завершается
// `persistCutSetupColumns()` по ВСЕЙ очереди.
//
// Теперь замороженный день исключается из входа ЦЕЛИКОМ (вся цепочка дробления, если хоть одно её
// звено стои́т в замороженном дне), а занятое им время отдаётся станку простоем
// (`excludedCutBlockedRanges`, #4434 п.2) — соседние дни считаются по честной ёмкости, и ничто не
// встаёт поверх замороженных заданий. Хранимый тайминг замороженных дней тоже не переписывается.
//
...
// ── 5) Цепочка дробления с ЗВЕНОМ в замороженном дне исключается целиком ────────────────────────
(function () {
    var cuts = [
        cut('H', 1, 480, 10),                              // голова — Вт (день 1, НЕ заморожен)
        cut('T', 2, 480, 6, { firstPartId: 'H' })          // продолжение — Ср (день 2, ЗАМОРОЖЕН)
    ];
    var isFrozen = function (planDate) {
        var t = Number(planDate) * 1000;
        return Math.floor((t - BASE) / 86400000) === 2;
    };
    var ids = P.frozenDayCutIds(cuts, isFrozen).sort();
    assertEqual(ids, ['H', 'T'],
        '#4436: цепочку исключаем ЦЕЛИКОМ — пере-планировать голову значит сдвинуть замороженное звено');
})();

// ── 6) Заморозки нет — ничего не исключаем ──────────────────────────────────────────────────────
(function () {
    var cuts = [cut('H', 1, 480, 10), cut('T', 2, 480, 6, { firstPartId: 'H' })];
    assertEqual(P.frozenDayCutIds(cuts, function () { return false; }), [],
        '#4436: без замороженных дней вход планировщика не урезается');
    assertEqual(P.frozenDayCutIds(cuts, null), [],
        '#4436: без предиката (таблицы «Заморозка» нет) — тоже');
})();
