...
//
// Теперь кандидатов ТРИ и они РАВНОПРАВНЫ — что выгоднее по объективу, то и берём:
//   C — перестановка ВНУТРИ дня (`intraDayImprovementOps`): состав дня, его номер и станок не
//       меняются, поэтому опоздания и конфликты с «Отпуском» те же, меняется только переналадка;
//       порядок считает тот же движок, что при генерации (`resequenceWithinDays`, #4139/#3996);
//   B — порядок/дни на текущих станках;   A — со сменой станка.
// При РАВНОМ объективе выигрывает менее «шумный»: C → B → A. Результат показывается обычным
// предпросмотром #4402.
//
...
// ── 6) Выбор кандидата: C/B/A равноправны — берём тот, чей объектив меньше ─────────────────────
(function () {
    var C = P.chooseOptimizeCandidate;
    // before=480. Локальная перестановка даёт 460, глобальные хуже → берём C (раньше был бы 'none').
    assertEqual(C(480, 490, 500, true, 460).action, 'C', '#4440: C лучше всех → C (раньше «план НЕ трогаем»)');
    // Глобальный B выгоднее локального C → берём B.
    assertEqual(C(480, 440, 500, true, 460).action, 'B', '#4440: B выгоднее C → B');
    // Смена станка выгоднее всех → A.
    assertEqual(C(480, 470, 450, true, 465).action, 'A', '#4440: A выгоднее всех → A');
    // Равный объектив → менее «шумный» кандидат: C (ничего не переезжает).
    assertEqual(C(480, 460, 460, true, 460).action, 'C', '#4440: при равенстве выигрывает C — ничего не переезжает');
    assertEqual(C(480, 460, 460, true, Infinity).action, 'B', 'при равенстве без C — по-прежнему B (без смены станка)');
    // Никто не лучше текущего → не трогаем.
    assertEqual(C(480, 490, 500, true, 480).action, 'none', '#4440: равенство текущему не применяем');
    // Инвариант: применённый план никогда не хуже текущего.
    [[480, 490, 500, true, 460], [480, 440, 500, true, 460], [480, 470, 450, true, 465],
     [480, 490, 500, true, 480]].forEach(function (a) {
        var r = C(a[0], a[1], a[2], a[3], a[4]);
        assert(r.obj <= a[0], 'инвариант: obj(' + r.obj + ') ≤ before(' + a[0] + ')');
    });
})();
